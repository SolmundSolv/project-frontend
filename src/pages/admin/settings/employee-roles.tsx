...
const EmployeeRolesPage = ({
  ...
  function handleDelete(id: string) {
    return async () => {
      const res = await fetch(
        `http://localhost:3001/product/employee/roles/${id}`,
        {
          method: "DELETE",
        }
      );
  ...
  async function handleCreate() {
    ...
    if (name) {
      const res = await fetch(`http://localhost:3001/employee/roles`, {
        method: "POST",
        body: JSON.stringify({
          name,
        }),
...
export const getServerSideProps: GetServerSideProps<{
  employeeRoles: { id: string; name: string; visibleOnCreate: boolean }[];
}> = async () => {
  const res = await fetch(`${process.env.FETCH_URL}/employee/roles`, {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
    },
  });
  const employeeRoles = await res.json();
  return {
    props: {
      employeeRoles: employeeRoles,
    },
...
const AuthEmployeeRoles = withAuth(EmployeeRolesPage);
AuthEmployeeRoles.getLayout = (page: ReactElement) => {
  return <AdminLayout>{page}</AdminLayout>;
};
export default AuthEmployeeRoles;
